...
import { Users, UserCheck, ShieldCheck, Zap, Sword, Award } from 'lucide-react';
...
export default function Programs() {
  return (
    <div className="pt-24 bg-brand-black">
      {/* Header */}
      <section className="py-20 container mx-auto px-6 text-center">
        <h1 className="font-heading text-7xl md:text-9xl text-white tracking-tighter mb-6 underline decoration-brand-red decoration-8 underline-offset-8">PROGRAMS</h1>
        <p className="text-xl text-gray-500 font-light tracking-widest max-w-3xl mx-auto uppercase">Authentic Kung Fu Training Pathways In Bangalore</p>
      </section>

      {/* Program Detailed View */}
      <section className="py-20 space-y-32">
        {programList.map((prog, idx) => (
          <div key={prog.id} className="container mx-auto px-6">
            <div className={`flex flex-col ${idx % 2 !== 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-20 items-center`}>
              ...
              <div className="lg:w-1/2 space-y-8">
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-brand-gold font-heading text-2xl tracking-widest italic">{prog.subtitle}</span>
                  </div>
                  <h2 className="font-heading text-6xl text-white tracking-tighter">{prog.title}</h2>
                </div>
                ...
                <div className="pt-6">
                  <Link 
                    to="/trial" 
                    className="inline-flex items-center gap-4 px-10 py-5 bg-brand-red text-white font-heading text-2xl tracking-[0.2em] hover:bg-brand-gold hover:text-brand-black transition-all"
                  >
      ...
      <section className="bg-brand-gold py-12">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8 text-brand-black">
          <h3 className="font-heading text-4xl tracking-widest underline decoration-brand-red decoration-4">NOT SURE WHERE TO START?</h3>
          <Link to="/contact" className="px-12 py-4 border-2 border-brand-black font-heading text-2xl tracking-widest hover:bg-brand-black hover:text-brand-gold transition-all">TALK TO A SENSEI</Link>
        </div>
      </section>
    </div>
  );
